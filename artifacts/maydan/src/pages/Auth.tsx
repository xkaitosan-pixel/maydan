...
export default function Auth() {
  ...
  const inputCls = "glass-card w-full h-12 px-4 text-sm text-right text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/60 focus:ring-2 focus:ring-primary/40 transition";
  ...
  return (
    <div className="min-h-screen gradient-hero particle-bg flex flex-col items-center justify-center p-5 text-center">
          ...
          <div className="space-y-3">
            ...
            <form onSubmit={handleEmailLogin} className="space-y-2.5">
              ...
              <button
                type="submit"
                disabled={!!loading}
                className="btn-primary-gold press-shrink w-full text-sm flex items-center justify-center gap-2 disabled:opacity-60"
                style={{ height: 52 }}
              >
            ...
            {/* Google — prominent white button with shadow */}
            <button
              onClick={handleGoogle}
              disabled={!!loading}
              className="press-shrink w-full flex items-center justify-center gap-3 font-bold text-sm transition disabled:opacity-50"
              style={{
                height: 56,
                borderRadius: 14,
                background: "#FFFFFF",
            ...
            >
              {loading === "google" ? (
                <span className="w-4 h-4 border-2 border-black/20 border-t-black/70 rounded-full animate-spin inline-block" />
              ) : (
                <>
                  <svg className="w-5 h-5 shrink-0" viewBox="0 0 24 24">
            ...
            <button
              onClick={handleApple}
              disabled={!!loading}
              className="press-shrink w-full h-12 rounded-xl flex items-center justify-center gap-2 font-bold text-white text-sm transition disabled:opacity-50"
              style={{ background: "#000" }}
            >
              {loading === "apple" ? <Spinner /> : <><AppleIcon /><span>تسجيل الدخول بـ Apple</span></>}
            </button>
            ...
            {/* Guest — outline / subtle */}
            <button
              onClick={playAsGuest}
              className="press-shrink w-full h-11 rounded-xl border-2 border-border bg-transparent text-muted-foreground hover:text-foreground hover:border-primary/40 font-bold text-sm transition-colors"
            >
              👤 العب كضيف
            </button>
      ...
      {/* Feature pills — first impression highlights */}
      <div className="flex flex-wrap justify-center gap-2 mt-7 max-w-sm">
        {[
          { icon: "⚔️", text: "تحدى أصدقاءك" },
          { icon: "🏆", text: "تصدر اللوحة" },
          { icon: "🎁", text: "اكسب جوائز" },
        ].map((f) => (
          <span
            key={f.text}
            className="glass-card text-foreground/90 text-xs px-3 py-1.5 inline-flex items-center gap-1.5"
