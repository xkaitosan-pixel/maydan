...
export default function Results() {
  ...
  function shareOnWhatsApp() {
    const catLabel = category ? `${category.icon} ${category.name}` : "المعرفة";
    const text =
      role === "creator"
        ? `⚔️ تحداك ${challenge!.creatorName} في ${catLabel}!\nنتيجته: ${creatorScore}/${total}\nهل تستطيع التغلب عليه؟\n${shareUrl}`
        : `⚔️ قبلت تحدي ${challenge!.creatorName} في ${catLabel}!\n${myName}: ${myScore}/${total} | ${opponentName}: ${opponentScore}/${total}\nجرب أنت:\n${shareUrl}`;
    ...
    <div className="min-h-screen gradient-hero flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {isCompleted ? (
          <div className="bg-card border border-border rounded-2xl p-5 fade-in-up">
            <div className="grid grid-cols-3 gap-4 items-center text-center">
              <div>
                <div
                  className="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-2 border-2"
          ...
          <div className="bg-card border border-border rounded-2xl p-5 text-center fade-in-up space-y-4">
            <div
              className="w-16 h-16 rounded-full flex items-center justify-center mx-auto gold-glow"
            ...
            <div className="flex justify-center gap-1">
              {[0, 1, 2].map(i => (
                <div key={i} className="w-2 h-2 rounded-full animate-bounce" style={{ background: category?.gradientFrom, animationDelay: `${i * 0.2}s` }} />
              ))}
            </div>
        ...
        <div className="space-y-3 fade-in-up pb-4">
          ...
          <Button onClick={shareOnWhatsApp} className="w-full h-12 font-bold rounded-xl text-white flex items-center justify-center gap-2" style={{ backgroundColor: "#25D366" }}>
          ...
          <Button onClick={() => navigate("/")} variant="outline" className="w-full h-12 rounded-xl border-border">
            🏠 الرئيسية
          </Button>

          <Button
            onClick={() => navigate("/create")}
            className="w-full h-12 rounded-xl text-white font-bold hover:opacity-90"
